-- Apply after 018. Keeps existing rounds/results and their saved scoring rules.
begin;
alter table public.open_play_rotation_settings drop constraint open_play_rotation_settings_minutes_check;
alter table public.open_play_rotation_settings add check(minutes between 0 and 180);
alter table public.open_play_rotation_settings drop constraint open_play_rotation_settings_target_check;
alter table public.open_play_rotation_settings add check(target between 1 and 199);
alter table public.open_play_matches add column timeout_a integer check(timeout_a between 0 and 199), add column timeout_b integer check(timeout_b between 0 and 199), add check((timeout_a is null)=(timeout_b is null));
create or replace function openly_private.lock_saved_rotation() returns trigger
language plpgsql set search_path='' as $$ begin
 if exists(select 1 from public.open_play_rounds where open_play_id=old.open_play_id and status<>'aborted')
 and (to_jsonb(old)-'courts'-'version') is distinct from (to_jsonb(new)-'courts'-'version') then raise exception 'Settings are locked after the first round is generated.';end if;
 return new;
end $$;
create or replace function openly_private.validate_rotation_finish(p_round uuid) returns void
language plpgsql set search_path='' as $$
declare r public.open_play_rounds%rowtype; m public.open_play_matches%rowtype; target integer; win integer; cap integer;
begin
 select * into r from public.open_play_rounds where id=p_round;
 ...
 for m in select * from public.open_play_matches where round_id=r.id loop
  if r.settings->>'style'='social' and r.kind='regular' then continue;end if;
  if m.a is null or m.b is null then raise exception 'Competitive rounds require scores.';end if;
  if m.a=m.b then raise exception 'A tied Competitive round must continue into overtime.';end if;
  if m.timeout_a is not null then
   if m.timeout_a=m.timeout_b then
    if not ((m.a=m.timeout_a+1 and m.b=m.timeout_b) or (m.b=m.timeout_b+1 and m.a=m.timeout_a)) then raise exception 'Overtime ends on the first point after the confirmed tie.';end if;
   elsif m.a<>m.timeout_a or m.b<>m.timeout_b then raise exception 'The confirmed timeout score is the final score.';
   end if;
  elsif not (greatest(m.a,m.b)>=target and (abs(m.a-m.b)>=win or greatest(m.a,m.b)=cap) and (cap is null or greatest(m.a,m.b)<=cap)) then
   if r.kind='regular' and r.settings->>'round_type'='timed' and (r.settings->>'minutes')::integer>0 and clock_timestamp()>=r.started_at+make_interval(mins=>(r.settings->>'minutes')::integer) then
    raise exception 'Confirm the score after the final rally before ending this timed round.';
   end if;
   raise exception 'Reach the target and win-by requirement before ending this round.';
  end if;
 end loop;
end $$;
revoke all on function openly_private.validate_rotation_finish(uuid) from public,anon,authenticated;
